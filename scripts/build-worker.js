...
// 3) آرایه را با قانون «۱۲۰ عنصر در هر خط» فرمت می‌کند
...
function buildPayloadArray(sourceCode) {
  const sourceBytes = Buffer.from(sourceCode, 'utf8');   // مرحله ۱
  const xored = xorWithKey16(sourceBytes);                // مرحله ۲
  const b64 = xored.toString('base64');                   // مرحله ۳
  const arr = [];
  for (let i = 0; i < b64.length; i++) {                   // مرحله ۴
...
function main() {
  ...
  if (!fs.existsSync(TEMPLATE_PATH)) {
    throw new Error(
      `worker.template.js پیدا نشد: ${TEMPLATE_PATH}\n` +
      `این فایل باید همان ساختار stub فعلی worker.js را داشته باشد، با این تفاوت که ` +
      `بلوک "const payload = ...;" با نشانگر ${PLACEHOLDER} جایگزین شده باشد.`
    );
  }
...
${arrText}
  ...
  console.log(`worker.js ساخته شد. طول آرایه‌ی payload: ${arr.length} عنصر.`);
}
